...
import {
  bulkUploadAccounts,
  downloadBulkAccountTemplate,
  type BulkUploadResult,
} from "@/lib/actions/bulk-account-upload";
...
function BulkUploadForm({ onClose }: { onClose: () => void }) {
  ...
  function handleUpload(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await bulkUploadAccounts(null, fd);
      setResult(res);
      if (res && res.ok && (res.created > 0 || res.orgsCreated > 0)) {
        router.refresh();
      }
    });
  }
  ...
  function handleTemplate() {
    startTemplate(async () => {
      const res = await downloadBulkAccountTemplate();
    ...
    <form onSubmit={handleUpload} className="flex flex-col gap-4">
      ...
      {result && result.ok && (
        <div className="flex flex-col gap-2">
          <div className="rounded-md border border-success/30 bg-success/10 px-3 py-2 text-sm text-fg">
            Created <strong>{result.created}</strong> account
            {result.created === 1 ? "" : "s"}
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        >
          ...
          {pending ? "Uploading…" : "Upload accounts"}
        </button>
      </div>
    </form>
  );
